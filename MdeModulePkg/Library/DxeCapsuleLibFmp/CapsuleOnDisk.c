  ...
  )
{
  ...
  // into "CoDRelocatedOption" var. It is used in next reboot after capsule is 
  // updated out of TCB to remove the TempCoDFile.
  //
  Status = gRT->SetVariable(
                   L"CoDRelocatedOption", 
                   &gEfiCapsuleVendorGuid, 
                   EFI_VARIABLE_NON_VOLATILE | EFI_VARIABLE_BOOTSERVICE_ACCESS,
                   sizeof (UINT16),
                   &LoadOptionNumber
...
/**
  For the platforms that support Capsule On Ram, reuse the Capsule On Ram to deliver capsule.
  ...
  )
{
  ...
  //
  // Clear "CoDRelocatedOption" variable
  //
  Status = gRT->SetVariable (
             L"CoDRelocatedOption",
             &gEfiCapsuleVendorGuid,
